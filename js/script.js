$(function() {
  "use strict";

  var testWithReplies = {
    "Javascript ...": {
      "это язык с динамической типизацией": true,
      "это язык со статической типизацией": false,
      "относится к императивным языкам программирования": false,
      "относится к декларативным языкам программирования": true
    },

    "Как скрипты Javascript можно встраивать в веб-страницу?": {
      "В теге <head> внутри тега script": true,
      "В разметке страницы внутри html-тегов": true,
      "В теге <body> внутри тега script": true,
      "Все варианты неверны": false
    },

    "Какое из утверждений верно?": {
      "Язык JavaScript произошел от Java": false,
      "Язык Java произошел от JavaScript": false,
      "Оба варианта неверны": true
    }
  };

  localStorage.setItem('testString', JSON.stringify(testWithReplies));
  var test = JSON.parse(localStorage.getItem('testString'));

  var rightAnswers = new Array; //создание массива ответов из объекта с вопросами

  var questions = Object.keys(test);

  for(var i = 0; i < questions.length; i++) {

    var answersObj = test[questions[i]];
    var answers = Object.keys(answersObj);

    var rightAnswersByNumber = new Array; //создание массива ответов для конкретного вопроса
      for(var j = 0; j < answers.length; j++) {
        var answerValue = answersObj[answers[j]];
        rightAnswersByNumber[j] = answerValue;
      }
    rightAnswers[i] = rightAnswersByNumber;
  };

  var html = $('#myTemplate').html();
  var content = tmpl(html, {
    data: test
  });
  $('body').append(content);

  $('.button').on('click', function(event) {
    event.preventDefault();
    var totalErrors = 0; //общее количество ошибок
    var questionError = 0; //количество вопросов, в которых сделаны ошибки
    var errorByIteration = 0; //вспомогательная переменная

    for(var i = 0; i < questions.length; i++) {
      
      for(var j = 0; j < $('.question').eq(i).children('.answer_list').children('li').length; j++) {
        var userAnswersByNumber = $('.question').eq(i).children('.answer_list').children('li').eq(j).children('label').children('input').prop('checked');

        if(userAnswersByNumber !== (rightAnswers[i])[j]) { //сравнение ответов пользователя с правильными ответами 
          totalErrors++;
        };

      };
      if(errorByIteration !== totalErrors) {
        questionError++;
        errorByIteration = totalErrors;
      };
    };
    showModal(totalErrors, questionError);
  });

  var overlay;
  var modal;

  function showModal(totalErrors, questionError) {
    overlay = $('<div class="overlay">');
    $('body').append(overlay);
    overlay.one('click', hideModal);

    if(totalErrors == 0) {
      modal = $('<div class="modal_window">Поздравляем! Вы успешно прошли тест!</div>')
    } else {  
      modal = $('<div class="modal_window">Вы сделали ' + totalErrors + ' ошибок в ' + questionError + ' вопросах!</div>')
    }
    $('body').append(modal);
  };

  function hideModal() {
    modal.remove();
    overlay.remove();
    $('input').prop('checked', false);
  };
});